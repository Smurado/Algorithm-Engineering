...
int main() {
    ...
    // With each iteration of the loop the array increases it's size up to 10^9.
    // The system has 16 threads so we will use 16 Threads/Runs.
    for (int64_t current_size = 10; current_size <= 1'000'000'000; current_size *= 10) {
        // threads used by min_max_quicksort and __gnu_parallel::sort
        omp_set_num_threads(16);
        std::cout << "\n======= Start with an array size of  " << current_size << "  ======= \n";

        // Fill the Array with values.
        std::vector<int64_t> master_data(current_size);
        Xoroshiro128Plus generator(seed);

        for (int64_t k = 0; k < current_size; ++k) {
            master_data[k] = (int64_t) generator();
        }

        // set the times
        double total_time_std = 0.0;
        double total_time_quicksort = 0.0;
        double total_time_gnu = 0.0;

        // perform 10 runs to get the average
        for (int r = 0; r < RUNS; r++) {

            // measure std::sort
            std::vector<int64_t> work_data = master_data;

            double start = omp_get_wtime();
            std::sort(work_data.begin(), work_data.end());
            double end = omp_get_wtime();
            total_time_std += end - start;

            // measure min_max_quicksort
            work_data = master_data;

            start = omp_get_wtime();
            min_max_quicksort(work_data.data(), current_size);
            end = omp_get_wtime();
            total_time_quicksort += end - start;

            // measure __gnu_parallel::sort
            work_data = master_data;

            start = omp_get_wtime();
            __gnu_parallel::sort(work_data.begin(), work_data.end());
            end = omp_get_wtime();
            total_time_gnu += end - start;
        }

        double avg_std = total_time_std / RUNS;
        double avg_quicksort = total_time_quicksort / RUNS;
        double avg_gnu = total_time_gnu / RUNS;

        std::cout << "std::sort avg time:          " << avg_std << " s\n";

        std::cout << "min_max_quicksort avg time:  " << avg_quicksort << " s\n";
        std::cout << " -> speedup over std::sort:  " << avg_std / avg_quicksort << "\n";

        std::cout << "__gnu_parallel avg time:     " << avg_gnu << " s\n";
        std::cout << " -> speedup over std::sort:  " << avg_std / avg_gnu << "\n";
    }
}

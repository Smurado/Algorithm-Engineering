...
One upcoming technology is PLC, which for now should only be used for archiving data because of its low endurance.
...
Voltage levels correspond with the bits able to be written in a cell. For one bit, you need two voltage levels; for two bits, one needs 4 voltage levels to distinguish the two bits' state (0,1).
...
This works best if the SSD is nearly or completely empty.
